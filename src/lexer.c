 ...
 */
static bool is_identifier_char(char ch, bool first_char)
{
...
kj_internal void lexer_init(lexer_info_t info, lexer_t *l)
{
...
kj_internal void lexer_deinit(lexer_t *l)
{
	kj_free(l->token_string, l->allocator);
}

kj_internal const char *lexer_token_to_string(token_t tok, char *buffer, uint buffer_size)
{
  switch (tok) {
    case tok_eos:
...
kj_internal const char *lexer_lookahead_to_string(lexer_t *l)
{
	return (l->lookahead == tok_eos) ? "end-of-stream" : l->token_string;
}

kj_internal token_t lex(lexer_t *l)
{
  l->lookahead = tok_eos;
      ...
      case '\'': {
...

        {
          ...
          case '9':
            ...
            if (decimal) {
              l->token.real = (koji_real)atof(l->token_string);
            } else {
              char *dummy;
              l->token.integer = (koji_integer)strtoll(l->token_string, &dummy, 10);
            }
