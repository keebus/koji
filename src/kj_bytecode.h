...
enum opcode {
  ...
  OP_DEBUG,    /* debug A, Bx       ; (temporary) prints Bx registers from R(A) */
};
...
static const int MAX_REG_VALUE = 255;
...
kj_forceinline bool opcode_has_target(enum opcode op) { return op <= OP_THIS; }

/* Encodes an instruction with arguments A and Bx. */
kj_forceinline instruction_t encode_ABx(enum opcode op, int A, int Bx)
{
   assert(A >= 0);
...
kj_forceinline instruction_t encode_ABC(enum opcode op, int A, int B, int C)
{
   assert(A >= 0);
...
kj_forceinline enum opcode decode_op(instruction_t i) { return i & 0x3f; }

/* Decodes an instruction argument A. */
kj_forceinline int decode_A(instruction_t i) { return (i >> 6) & 0xff; }

/* Decodes an instructions argument B. */
kj_forceinline int decode_B(instruction_t i) { return ((int)i << 9) >> 23; }

/* Decodes an instruction argument C. */
kj_forceinline int decode_C(instruction_t i) { return (int)i >> 23; }

/* Decodes an instruction argument Bx. */
kj_forceinline int decode_Bx(instruction_t i) { return (int)i >> 14; }

/* Sets instruction argument A. */
kj_forceinline void replace_A(instruction_t* i, int A)
{
   assert(A >= 0);
   *i = (*i & 0xFFFFC03F) | (A << 6);
}
...
kj_forceinline void replace_Bx(instruction_t* i, int Bx)
{
   *i = (*i & 0x3FFF) | (Bx << 14);
}

/* Set instruction argument C */
kj_forceinline void replace_C(instruction_t* i, int C)
{
   *i = (*i & 0x7FFFFF) | (C << 23);
}
