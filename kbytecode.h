...
enum opcode {
	...
	OP_TESTSET,  /* testset A, B, C  ; if R(B) == (bool)C then R(A)
	                  = R(B) else jump 1 */
	...
	OP_EQ,      /* eq A, B, C    ; if (R(A) == R(B)) == (bool)C then nothing
	               else jump 1 */
	OP_LT,      /* lt A, B, C    ; if (R(A) < R(B)) == (bool)C then nothing
	               else jump 1 */
	OP_LTE,     /* lte A, B, C   ; if (R(A) <= R(B)) == (bool)C then nothing
	               else jump 1 */
	OP_CALL,    /* call A, B, C  ; call object R(C) with B arguments starting at
	               R(A) */
	OP_MCALL,   /* mcall A, B, C ; call object R(A - 1) method with name R(B)
	               with C arguments from R(A) on */
...
#define MAX_ABC_VALUE 255

/* Maximum value Bx can hold (positive or negative) */
#define MAX_BX_VALUE 131071
...
struct prototype {
	...
	uint32_t protos_beg;
	/* Number of instructions in this prototype */
	uint16_t ninstrs;
	/* Number of constants in this prototype */
	uint16_t nconsts;
	/* Number of prototypes in this prototype */
	uint16_t nprotos;
	/* Number of arguments this prototype requires */
	uint16_t nargs;
	/* Number of registers the prototype requires */
	uint16_t nregs;
	/* Unused field for padding */
	uint16_t unused;
};
...
/*
 * Returns the size in bytes of a module with specified parameters.
 */
inline uint32_t
module_sizeof(uint16_t instrs_len, uint16_t consts_len, uint16_t protos_len)
{
	return sizeof(struct module) + sizeof(instr_t) * instrs_len +
	       sizeof(union value) * consts_len +
	       sizeof(struct prototype) * protos_len;
}

/*
 * Releases a reference to specified module and frees it if it has no more
 * references.
 */
kintern void
module_unref(struct module *, struct koji_allocator *);

/*
 * Dumps human readable information about specified module to stdout (e.g.
 * instruction disassembly, constants, etc.)
 */
kintern void
module_dump(struct module const *);
