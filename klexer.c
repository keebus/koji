...
lex_skip(struct lexer *l)
{
	...
	++l->sourceloc.column;
	l->curr = (char)l->source->read(l->source->user);
	return l->curr;
}
 ...
 */
static int32_t
lex_push(struct lexer *l)
{
	if (l->tokstrlen + 2 > l->tokstrbuflen) {
		l->tokstr = l->allocator.reallocate(
		    l->tokstr, l->tokstrbuflen, l->tokstrbuflen * 2, l->allocator.user);
		l->tokstrbuflen *= 2;
	}
 ...
 */
static void
lex_tokstr_clear(struct lexer *l)
{
	l->tokstrlen = 0;
	l->tokstr[0] = '\0';
}
 ...
 */
static int32_t
lex_accept_str(struct lexer *l, const char *str)
{
	while (l->curr == *str) {
		lex_push(l);
		++str;
 ...
 */
static int32_t
lex_accept_char(struct lexer *l, char ch)
{
	if (l->curr == ch) {
		lex_push(l);
		return true;
 ...
 */
static token_t
lex_scan_id(struct lexer *l, bool first_char)
{
...
kintern void
lex_init(struct lexer *l, struct issue_handler *ih, struct koji_source *src,
         struct koji_allocator *alloc)
{
	...
	l->source = src;
	l->tokstrbuflen = 128;
	l->tokstr = kalloc(char, l->tokstrbuflen, &l->allocator);
	l->tokstrlen = 0;
	l->sourceloc.name = src->name;
	l->sourceloc.line = 1;
	l->sourceloc.column = 0;
	l->newline = 0;
	l->curr = 0;
...
kintern void
lex_deinit(struct lexer *l)
{
	kfree(l->tokstr, l->tokstrbuflen, &l->allocator);
}

kintern const char *
lex_tok_pretty_str(token_t tok, char *buffer, int32_t buffer_size)
{
#define TOKEN_CASE(enum_, str)                                                 \
	case enum_:                                                                 \
		return str;
	switch (tok) {
		TOKENS(TOKEN_CASE);
...
kintern const char *
lex_tok_ahead_pretty_str(struct lexer *l)
{
