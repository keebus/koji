...
/*
 * Describes a branch. Todo: add more details.
 */
struct branch {
	struct branch *next;
	int32_t instridx;
 ...
 */
struct protoinfo {
	/* Index of the first instructions in prototype */
	int32_t instrs_beg;
	/* Index after last instruction in prototype */
	int32_t instrs_end;
	/* Index of the first constant in prototype */
	int32_t consts_beg;
	/* Index after the last constant in prototype */
	int32_t consts_end;
	/* Index of first child prototype in prototype */
	int32_t protos_beg;
	/* Index after first child prototype in prototype */
	int32_t protos_end;
	/* Number of locals in curr prototype */
	int32_t nlocals;
	/* Index of the next free register for locals */
	loc_t temp;
	/* Total number of registers necessary to execute this prototype */
	int32_t nregs;
	/* Array of local variables */
	struct local *locals;
	/* List of branches that eval to true */
	struct branch *branch_true;
	/* List of branches that eval to false */
	struct branch *branch_false;
};
...
struct compiler {
	...
	/* The lexer used to scan tokens from input source. */
	struct lexer lex;
	/* A simple linear allocator for temporary memory. */
	struct scratch_page *scratch;
	/* Cursor in the first free byte in the scratch allocator. */
	struct scratch_pos scratchpos;
	/* Total number of instructions in module */
	uint32_t instrs_size;
	/* Length of `instrs` array. */
	uint32_t instrs_len;
	/* Buffer of all instructions in the prototype. */
	instr_t *instrs;
	/* Total number of constants in module */
	uint32_t consts_size;
	/* Length of `consts` array. */
	uint32_t consts_len;
	/* Buffer of constants in the prototype. */
	union value *consts;
	/* Total number of prototypes in module */
	uint32_t protos_size;
	/* Length of `protos` array. */
	uint32_t protos_len;
	/* Buffer of prototypes in the prototype. */
	struct prototype *protos;
	/* The current prototype being compiled */
	struct protoinfo pi;
};

#define compiler_scratch_alloc(c, type)                                        \
	scratch_alloc(&c->scratchpos, type, &c->lex.allocator)
 ...
 */
static const char *EXPR_TYPE_TO_STRING[] = {"nil",   "bool", "number", "string",
 ...
 */
static const enum opcode BINOP_TO_OPCODE[] = {0,      OP_MUL, OP_DIV,
                                              OP_MOD, OP_ADD, OP_SUB};
...
struct expr_state {
	struct branch
	    *branch_true; /* index of the first branch to true in exprstate*/
...
expr_negate(struct expr e)
{
	...
	case EXPR_BOOL:
		return expr_bool(!e.val.boole);
	case EXPR_NUMBER:
		return expr_bool(!expr_tobool(e));
	default:
...
check(struct compiler *c, token_t tok)
{
	if (!peek(c, tok)) {
		char buffer[64];
		error(c->lex.issue_handler, c->lex.sourceloc, "missing %s before '%s'.",
		      lex_tok_pretty_str(tok, buffer, 64),
		      lex_tok_ahead_pretty_str(&c->lex));
	}
}
...
{
	struct branch *br = compiler_scratch_alloc(c, struct branch);
	br->next = *head;
	*head = br;
	return &br->instridx;
}
 ...
 * using the new temporary you must restore the temporary register [c->pi.temp]
...
emit(struct compiler *c, instr_t instr)
{
	...
	/* push the instruction in the instrs master array; note that the actual the
	 * actual number of instructions in this prototype is pi.instrs_end */
	array_push(&c->instrs, &c->pi.instrs_end, &c->instrs_len,
	           &c->lex.allocator) = instr;
}
...
{
	struct local *local =
	    scratch_alloc_ex(&c->scratchpos, sizeof(struct local) + idlen + 1,
	                     kalignof(struct local), &c->lex.allocator);
 ...
 * enough, otherwise to a temporary into which the constant is moved with a move
 * instruction.
 */
static struct expr
...
const_fetch_num(struct compiler *c, loc_t target_hint, koji_number_t num)
{
	...
	union value *cnst =
	    &array_seqpush(&c->consts, &c->pi.consts_end, &c->lex.allocator);
                ...
                int32_t len)
{
	...
	for (int32_t i = 0; i < c->pi.consts_end; ++i) {
		...
		struct string *string = string_getobj(*cnst);
	...
	cnst = &array_seqpush(&c->consts, &c->pi.consts_end, &c->lex.allocator);

	/* create a new string */
	*cnst = value_new_string(c->cls_string, &c->lex.allocator, len);
...
expr_compile(struct compiler *c, struct expr e, int32_t target_hint)
{
	...
	case EXPR_LTE:
		...
		emit(c, encode_ABC(OP_EQ + e.type - EXPR_EQ, e.val.comp.lhs, e.positive,
		                   e.val.comp.rhs));
 ...
 */
static struct expr
expr_compile_binary(struct compiler *c, struct sourceloc op_sloc, enum binop op,
...
#define DEFAULT_ARITH_BINOP(opchar)                                            \
	...
	case BINOP_LOGICAL_AND:
		return (expr_isconst(lhs.type) && !expr_tobool(lhs)) ? expr_bool(false)
		                                                     : rhs;

	case BINOP_LOGICAL_OR:
		return (expr_isconst(lhs.type) && expr_tobool(lhs)) ? expr_bool(true)
		                                                    : rhs;
...
error: /* the binary operation between lhs and rhs is invalid */
	error(c->lex.issue_handler, op_sloc,
	      "cannot make binary operation '%s' "
	      "between values of type '%s' and '%s'.",
	      BINOP_TO_STR[op], EXPR_TYPE_TO_STRING[lhs.type],
                   ...
                   struct expr lhs)
{
	...
	case EXPR_LOCATION:
		if ((!lhs.positive) == es->negated) {
			emit(c, encode_ABC(OP_TESTSET, MAX_ABC_VALUE, lhs.val.loc, testval));
		} else {
           ...
           loc_t target_hint, bool movetotarget)
{
	...
	int32_t rhs_move_jump_idx =
	    0;                            /* index of the instruction to jump to that
	                                                                     moves rhs to lhs */
...
scan_id(struct compiler *c, loc_t target_hint)
{
	...
	struct expr expr =
	    const_fetch_str(c, target_hint, c->lex.tokstr, c->lex.tokstrlen);
	lex(c);
	return expr;
}

/*
 * Pushes the current prototype to the module being compiled.
 */
static void
push_prototype(struct compiler *c, int32_t nargs, struct protoinfo const *pi)
{
	struct prototype p;
	p.instrs_beg = pi->instrs_beg;
	p.consts_beg = pi->consts_beg;
	p.protos_beg = pi->protos_beg;
	p.ninstrs = c->pi.instrs_end - c->pi.instrs_beg;
	p.nconsts = c->pi.consts_end - c->pi.consts_beg;
	p.nprotos = c->pi.protos_end - c->pi.protos_beg;
	p.nargs = nargs;
	p.nregs = c->pi.nregs;
	p.nargs = nargs;
	c->pi = *pi;
	array_push(&c->protos, &c->pi.protos_end, &c->protos_len,
	           &c->lex.allocator) = p;

	c->instrs_size += p.ninstrs;
	c->consts_size += p.nconsts;
	++c->protos_size;
}
...
parse_closure(struct compiler *c)
{
	...
	c->pi =
	    (struct protoinfo){c->pi.instrs_end, c->pi.instrs_end, c->pi.consts_end,
	                       c->pi.consts_end, c->pi.protos_end, c->pi.protos_end};
		...
		do {
			...
			if (peek(c, tok_identifier)) {
				...
				if (accept(c, ':')) {
					...
					error(c->lex.issue_handler, sl,
					      "cannot leave key undefined "
			...
			if (has_key) {
				/* parse value */
				value = parse_exprto(c, c->pi.temp, false);
			} else {
				value = key;
				/* parse key */
				key = expr_compile(c, expr_num(index++), c->pi.temp);
			}
...
parse_primary_expr(struct compiler *c, struct expr_state *es)
{
	...
	case kw_false:
		lex(c);
		expr = expr_bool(false);
		break;

	case tok_number:
		lex(c);
		expr = expr_num(c->lex.toknum);
		break;
...
parse_cond(struct compiler *c, bool testval)
{
	...
	if (expr_iscompare(expr.type)) {
		emit(c, encode_ABC(OP_EQ + expr.type - EXPR_EQ, expr.val.comp.lhs,
		                   expr.val.comp.rhs, expr.positive ^ !testval));
	} else {
...
parse_stmt_if(struct compiler *c)
{
	...
	if (accept(c, kw_else)) {
		...
		replace_Bx(c->instrs + exitjmpidx, offset_to_next_instr(c, exitjmpidx));
	} else {
...
{
	/* parse the main prototype and push it to the list */
	parse_prototype_body(c);
	expect(c, tok_eos);
	push_prototype(c, 0, &c->pi);

	/* compute the module byte size */
	const uint32_t modsize =
	    module_sizeof(c->instrs_size, c->consts_size, c->protos_size);

	/* allocate the module */
	struct module *mod =
	    c->lex.allocator.allocate(modsize, c->lex.allocator.user);

	/* setup the module parameters */
	mod->refs = 0;
	mod->instrs_len = c->instrs_size;
	mod->consts_len = c->consts_size;
	mod->protos_len = c->protos_size;
	mod->instrs = (instr_t *)((char *)mod + sizeof(struct module));
	mod->consts =
	    (union value *)((char *)mod->instrs + sizeof(instr_t) * mod->instrs_len);

	/* copy instructions, constants and prototype infos into the module */
	memcpy(mod->instrs, c->instrs, sizeof(*mod->instrs) * mod->instrs_len);
	memcpy(mod->consts, c->consts, sizeof(*mod->consts) * mod->consts_len);
	memcpy(mod->protos, c->protos, sizeof(*mod->protos) * mod->protos_len);

	/* set each prototype in the module to reference the module itself */
	for (int i = 0; i < mod->protos_len; ++i)
		mod->protos[i].module = mod;
...
compile(struct compile_info *ci, struct prototype **proto)
{
	struct compiler c = {0};

	/* redirect the error handler jump buffer here so that we can cleanup the
	 * state */
	enum koji_result result = setjmp(ci->issue_handler.error_jmpbuf);
	if (result)
		goto error;

	/* initialize the lexer */
	lex_init(&c.lex, &ci->issue_handler, ci->source, &ci->allocator);

	/* initialize the scratch memory allocator */
	c.scratch = scratch_create(&c.scratchpos, &ci->allocator);

	/* initialize the rest of the compiler arrays and buffers */
	c.instrs_len = 128;
	c.instrs = kalloc(instr_t, c.instrs_len, &ci->allocator);
	c.consts_len = 64;
	c.consts = kalloc(union value, c.consts_len, &ci->allocator);
	c.protos_len = 8;
	c.protos = kalloc(struct prototype, c.protos_len, &ci->allocator);

	/* kick off compilation! */
	struct module *mod = parse_module(&c);
	mod->refs = 1; /* since we're returning one prototype reference, bump up the
	                  module ref count to 1 */

	/* if still here, it means compilation was successful */
	*proto = &mod->protos[0];
	goto cleanup;

error:
	/* destroy all constants */
	for (uint32_t i = 0; i < c.consts_len; ++i)
		const_destroy(c.consts[i], &ci->allocator);
	/* todo: cleanup prototypes
	for (int32_t i = 0; i < comp.pi.protos_end; ++i)
	   prototype_release(comp.protos[i], &info->allocator);
	*/
cleanup:
	kfree(c.protos, c.protos_len, &ci->allocator);
	kfree(c.consts, c.consts_len, &ci->allocator);
	kfree(c.instrs, c.instrs_len, &ci->allocator);
	scratch_delete(c.scratch, &ci->allocator);
	lex_deinit(&c.lex);
	return result;
}
