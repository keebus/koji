...
class_op_invalid(struct vm *vm, struct object *obj, enum class_op_id op,
                 union value arg1, union value arg2)
{
	...
	if (op == CLASS_OP_UNM) {
		...
		if (value_isobj(arg1)) {
			...
			int32_t total_len = snprintf((char *)arg_type_str, buflen,
			                             "'%s' object", argobj->class->name);
...
kintern union class_op_result
class_op_default_compare(struct vm *vm, struct object *obj, enum class_op_id op,
                         union value arg1, union value arg2)
{
	assert(!value_isnil(arg1) && value_isnil(arg2));
	struct object *argobj = value_getobj(arg1);
	union class_op_result res = {
	    1}; /* objects are always greater than any
	                                               other value type */
...
kintern union class_op_result
class_op_default_hash(struct vm *vm, struct object *obj, enum class_op_id op,
                      union value arg1, union value arg2)
{
	assert(value_isnil(arg1) && value_isnil(arg2));
	union class_op_result res;
...
kintern void
class_init_default(struct class *cls, const char *name, struct class *class_cls)
{
	...
	cls->operator[CLASS_OP_COMPARE] = class_op_default_compare;
	cls->operator[CLASS_OP_HASH] = class_op_default_hash;
...
{
	class_init_default(cls, "builtin_class", cls);
