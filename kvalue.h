...
static const union value VALUE_NIL = {.bits = BITS_NAN_MASK};
...
{
	if (value_isbool(val))
		return value_getbool(val);
	else if (value_isnum(val))
		return val.num != 0;
	else if (value_isnil(val))
		return false;
	return true;
}
